...
#include "USB_usb_standard.h"

#define USB_EP0_SIZE 64
// for some reason doesn't work with 8 (only sends first 8 bytes
//  of device descriptor, which is not enough to set the endpoint
//  size?)
...
/// Callback on Start-of-Frame interrupt
...
uint16_t usb_cb_get_descriptor(uint8_t type, uint8_t index, const uint8_t** descriptor_ptr, uint16_t offset);
...
/// Disable USB module completely (needs usb_init to enable again)
...
void usb_ep_enable(usb_ep ep, uint8_t type, usb_size pkt_size);
...
void usb_ep_wait_for_ready(usb_ep ep);

/// Returns true if received an OUT transaction
bool usb_ep_out_complete(usb_ep ep);

/// Returns true if the previous IN transfer is done
bool usb_ep_in_ready(usb_ep ep);

/// Returns true if there is a completion pending on the endpoint
bool usb_ep_pending(usb_ep ep);

void usb_ep_wait_for_pending(uint8_t ep);

/// Returns the last frame number received on Start-Of-Frame packet
/// Notes: 11bits, incremented every ms (so covers 2048ms)
/// Needs to be enabled with STFRNUM bit in USB.CTRLA
uint16_t usb_framenum(void);
