...
#include "USB_usb_xmega_internal.h"
...
void usb_reset(){
  ...
  // enable USB, set high speed and store SOF frame number in USB.FRAMENUM. Do no enable FIFO.
...
void usb_disable(void) {
  usb_configuration = 0;
  usb_detach();
  USB.CTRLA = 0; // the main point USB_ENABLE_bm -> 0
}
...
const uint8_t* usb_ep0_from_progmem(const uint8_t* addr, uint16_t size) {
  ...
  uint16_t remaining = (size <= USB_EP0_SIZE ? size : USB_EP0_SIZE); // only go up to EP0_SIZE
...
inline void usb_ep_wait_for_ready(usb_ep ep){
  while (!usb_ep_ready(ep)) {};
}

inline bool usb_ep_out_complete(uint8_t ep) {
  _USB_EP(ep);
  return (e->STATUS & (USB_EP_BUSNACK0_bm | USB_EP_TRNCOMPL0_bm));
}

inline bool usb_ep_in_ready(uint8_t ep) {
  _USB_EP(ep);
...
inline size_t usb_ep_out_length(uint8_t ep){
  _USB_EP(ep);
  return e->CNT;
}
...
uint16_t usb_framenum(void) {
  return *((uint16_t*)((uint8_t*)USB.EPPTR + ((usb_num_endpoints+1)<<4)));
}
